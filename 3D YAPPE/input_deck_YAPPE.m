...
% %     s.input.r_pts = 600; %number of pts along radial direction    
    s.input.x_extent = .1; %x-axis window length in cm from edge to edge
    s.input.x_pts = 150; %number of pts along x direction
    s.input.y_extent = .1; %y-axis window length in cm from edge to edge
    s.input.y_pts = 150; %number of pts along y direction
...
% %     s.input.infield.waist = 40e-4; %gaussian waist in cm (used in gauss)
    ...
    s.input.outpath = 'C:\Users\Pinky\Documents\MATLAB\Jesse GPU\3D YAPPE\Outputs\Run35'; %output path
