...
function[] = batch_runner_YAPPE()
    ...
    s.f.E_out(:,:,1) = gather(s.f.E); %first entry is input field    
%     s.f.rho_out_r = gpuArray(zeros(s.input.r_pts,length(s.g.zout))); %initialize output plasma matrix
%     s.f.rho_out_xi = gpuArray(zeros(s.input.xi_pts,length(s.g.zout))); %initialize output plasma matrix
    ...
    for m = 2:length(s.g.zout)
        ...
        initial_Ef = gpuArray(s.f.Ef(:)); %convert e-field matrix into vector for ode45 solve
        [~,x] = ode45GPUv2( @(t,y) PNL_step_YAPPE(t,y), [0 s.input.outperiod/2 s.input.outperiod], initial_Ef, opts); %solve ODE
        ...
        s.f.E_out(:,:,m) = gather(ifft(s.f.Ef,[],2));
        s.f.E_out(:,:,m) = gather(s.f.H*s.f.E_out(:,:,m));
