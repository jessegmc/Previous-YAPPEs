function [tf,yf] = ode45GPU(ode,tspan,y0,options,varargin)
g = gpuDevice;
mem(1,1) = g.AvailableMemory;
...
classT0 = class(t0);
classY0 = classUnderlying(y0);
classF0 = classUnderlying(f0);
...
if ntspan>2
    ...
    yout = gpuArray(zeros(neq,ntspan,dataType));
else
...
f = gpuArray(zeros(neq,7,dataType));
mem(1,5) = g.AvailableMemory;
...
while ~done
    ...
    while true
        ...
        for i = 2:6
            ...
            %f(:,i) = gpuArray(rand(600*600*300,1,'single'));
        end
        ...
        %f(:,7) = gpuArray(rand(600*600*300,1,'single'));
        nfevals = nfevals+6;
