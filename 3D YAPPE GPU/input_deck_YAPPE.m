...
% %     s.input.r_pts = 600; %number of pts along radial direction    
    s.input.x_extent = .07; %x-axis window length in cm from edge to edge
    s.input.x_pts = 100; %number of pts along x direction
    s.input.y_extent = .07; %y-axis window length in cm from edge to edge
...
% %     s.input.infield.waist = 40e-4; %gaussian waist in cm (used in gauss)
    ...
    s.input.infield.energ = 0.5e-6; %beam energy in J (used in gauss)
    ...
    s.input.outpath = 'R:\Jesse GPU\MATLAB\3D GPU YAPPE\Outputs\Run19\'; %output path
